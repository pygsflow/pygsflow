...
class Mfvtk(object):
    ...
    def __init__(self, mf=None, vtkname="mf_", out_folder = None,
                 mf_pkg=[], all=True,
    ...
    def __get_dummy_3d_model(self, ibound= None):
        ...
        top = dis.top.array
        botm = dis.botm.array
        dis3 = flopy.modflow.ModflowDis(mf3, nlay=nlay, nrow=nrow, ncol=ncol,
                                        nper=1, delr=delr, delc=delc, top=top, botm=botm)
        if not(ibound is None):
            ib = ibound
        else:
    ...
    @staticmethod
    def mf_to_vtk(vtkname="mf_", mfname=None, mf_pkg=[], all=True, out_folder=None,
                  shared_vertex=True, ibound_filter=True):
        ...
        mfvtk = Mfvtk(mf=mf, vtkname=vtk_nm, mf_pkg=[], all=all,
    ...
    def __write(self):
        ...
        for obj in self.vtk_objects.keys():
            ...
            self.vtk_objects[obj].write(shared_vertex=shared_vertex,
                                        ibound_filter=ibound_filter)



class Gsflowvtk():
    def __init__(self, gs = None, vtkname="gs_", mf_pkg=[], mfall=True, out_folder=None,
                      shared_vertex=True, ibound_filter=True):
        if out_folder:
            vtk_nm = os.path.join(out_folder, vtkname)
        else:
    ...
    def write_vtk(self):
        ...
        if mf:
            mfv = Mfvtk(mf = mf, vtkname = self.vtkname,mf_pkg=self.mf_pkg,all = self.mfall,
                        out_folder=self.out_folder,shared_vertex=self.shared_vertex,
                        ibound_filter=self.ibound_filter )
            mfv.write_vtk()
    ...
    def write_prms_vtk(self):
        ...
        vtkfile.write(shared_vertex=self.shared_vertex, ibound_filter=self.ibound_filter)
