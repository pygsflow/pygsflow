...
class Grid():
    ...
    - The grid is prism in 3D, meaning that for any cell top and bottom 
      faces are parallel and have the same top/bottom2D shape
    ...
    def __init__(self):  
        ...
        self._nnode = None       
        self.vertices = []  # dataframe with columns x, y, z
        self.node_data = []  # dataframe: inode, x, y, z, lay, vertex_indices
        self._geometry = None  # dataframe with columns node_id, geometry, 
                               # top, botom
        self._geometry2D = None  # a list of shapely polygons
        self._nlays = None
        
    @property
    def nlays(self):
        if self._nlays is not None:
            return self._nlays
        self._nlays = len(self.node_data['lay'].unique())
        return self._nlays
    ...
    @property
    def nvertex(self):
        if self._nvertex is not None:
            return self._nvertex
        self._nvertex = len(self.vertices)
        return self._nvertex


    def _create_geometry(self):
        """
        Create a dataframe with columns node_id, geometry
        """
        ...
        for _, row in self.node_data.iterrows():          
            ...
            # Convert 1-based vertex IDs to 0-based indices for pandas
            vertex_indices_0based = [vid - 1 for vid in vertex_indices]
            geometry = self.vertices.iloc[vertex_indices_0based]
            ...
            self._geometry.loc[node_id, 'zc'] = ((geometry['z'].max() + 
                                                 geometry['z'].min()) / 2)
            self._geometry.loc[node_id, 'inode'] = node_id + 1

        self._geometry['layer'] = None
        for idx, row in self._geometry.iterrows():
            # Check if this row's layer has already been assigned
            if self._geometry.loc[idx, 'layer'] is not None:
                continue
            x = row['xc']
            y = row['yc']
            top = row['top']
    ...
    def geometry2D(self, layer: int = 0) -> List[Polygon]:
        """
        Get the 2D geometry for a given layer.
        Args:
            layer: Layer number
        Returns:
            List of shapely polygons
        """
        layer_node_data = self.node_data[self.node_data['lay']==layer]
        layer_geometry = []
        for _, row in layer_node_data.iterrows():
            vertex_indices = row['vertex_indices']
            vertex_indices_0based = [vid - 1 for vid in vertex_indices]
            vts = self.vertices.iloc[vertex_indices_0based]
            vts =  vts.drop_duplicates(subset=['x', 'y'], keep='first')
            vts =vts[['x', 'y']].values.tolist()

            layer_geometry.append(Polygon(vts))
    ...
    def from_shapefile(self, filename: str, top, thickness) -> None:
        ...
        for node_idx in range(self.nnode):
            ...
            for layer_idx in range(self.nlays):
                ...
                node_data.append({
                    'inode': node_idx * self.nlays + layer_idx + 1,
                    'x': center_x,
                    'y': center_y,
                    'z': center_z,
                    'lay': layer_idx + 1,
    ...
    @classmethod
    def from_polygon_list(cls, polygon_list: List[Polygon]) -> 'Grid':
        """
        ...
        for ipolygon, polygon in enumerate(polygon_list):             
            ...
            node_data.append({
                'inode': ipolygon + 1,  # 1-based indexing
                'x': xc,
                'y': yc,
